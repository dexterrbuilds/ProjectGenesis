...
import {CLOSED_PERMISSIONS,serializedDigest} from '../../core/v2/identity.ts';
import type {Organism,Decision} from '../../core/contracts.ts';
const rows=JSON.parse(readFileSync('outputs/awakening-preparation/PRIVATE_BEFORE.json','utf8')).rows;
...
const prepared=await prepareCycle(o,life,brain,new ProductFallback(),{id:'fixture-event',kind:'digital-event',source:'local-review-fixture',observedAt:'2026-09-20T12:00:00Z'},CLOSED_PERMISSIONS,'2026-09-20T12:00:00Z','fixture-v2');
...
for(const [path,value] of Object.entries(routes)){if(JSON.stringify(value).includes('CANARY'))throw new Error('Public leakage');if(path!=='/api/brain')writeFileSync('outputs/awakening-preparation/UI_'+path.split('/').pop()+'.json',JSON.stringify(value,null,2));}

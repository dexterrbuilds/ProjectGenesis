...
## Resource review and external evidence

`manage_resources` is a product action permitted under APPROACH/AVOID. It reviews existing simulated cash and projects without moving money, then yields novelty 0.55 and uncertainty 0.35 near the reserve (0.10 otherwise). These explicit development-world stimuli are not a model of worm financial reasoning. Planner selection can avoid futile repeated spending while remaining subordinate to the decoded drive.

A configured Solana wallet observation is timestamped external evidence for the planner and public Money view. It does not change anatomical data, neural state, sensory mappings or simulated capital. A balance increase alone is not classified as trading-fee income. External reads, product energy and LLM rationales remain distinct from the recorded neural response.

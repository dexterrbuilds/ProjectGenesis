"""Summarize already frozen fits and completed replay controls; no optimization."""
import hashlib,json
...
def main():
 f=read('CALIBRATION_FROZEN.json');v=read('VALIDATION.json');i=read('IDENTIFICATION_FROZEN.json');r=read('REPLAY_COMPARISON.json');t=read('REPLAY_TRIAL10.json')
 counts=dict(Counter(x['status'] for x in read('PARAMETER_EVIDENCE.json')))
 summary={'classification':'PARTIAL PHYSIOLOGICAL CONSTRAINT','audited_entries':sum(counts.values()),'parameter_evidence_counts':counts,
  'frozen_apl_sha256':hashlib.sha256((H/'CALIBRATION_FROZEN.json').read_bytes()).hexdigest(),
  'APL_holdout_MSE':v['mse'],'APL_local_coefficients':f['local_transfer'],
  'APL_recording_bootstrap_intervals':f['recording_bootstrap_95pct_unbounded'],
  'Hige_integrated_depression':i['effective_integrated_depression'],'Hige_joint_rank':i['joint_parameter_jacobian_rank'],
  'neural_parameter_changes':{},'calibrated_neural_kernel':None,'calibrated_replay_outcomes':None,
  'null_transfer_exact_replays':sum(x['trained_state_exact'] for x in r['runs']),
  'stage3_original_metrics':t,'preserved_classifications':{'Stage1':'PASS','Stage2':'PARTIAL-INCONCLUSIVE','Stage3':'PARTIAL-INCONCLUSIVE','Architecture':'INSUFFICIENT EVIDENCE'}}
 (H/'SUMMARY.json').write_text(json.dumps(summary,indent=2)+'\n')
 stage_rows=[]
 for s in (1,2,3):
  rr=[x for x in r['runs'] if x['run'].startswith(f's{s}-')]
  stage_rows.append(f"| {s} | {len(rr)} | {min(x['wall_seconds'] for x in rr):.2f}–{max(x['wall_seconds'] for x in rr):.2f} s | {min(x['peak_process_rss_bytes'] for x in rr)/2**20:.1f}–{max(x['peak_process_rss_bytes'] for x in rr)/2**20:.1f} MiB |")
 text='''# Drosophila Physiology Calibration and Model-Identification Study

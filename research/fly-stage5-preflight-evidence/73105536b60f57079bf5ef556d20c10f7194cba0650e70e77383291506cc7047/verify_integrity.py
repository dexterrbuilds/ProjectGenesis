...
def filecheck(p,h):check(str(p.relative_to(ROOT)),sha(p),h)
try:
 candidates=[]
 ...
 for name,d in dep.items():
  ...
  if name=='genesis-brain-spec-v0.2':
   ...
   filecheck(base/'PACKAGE_MANIFEST.json',d['package_sha256'])
   for p,h in load(base/'PACKAGE_MANIFEST.json')['files'].items():filecheck(base/p,h)
  if name=='fly-stage1':
   em=ROOT/d['evidence_manifest_path'];filecheck(em,d['evidence_manifest_sha256'])
   for p,h in load(em)['files'].items():filecheck(em.parent/p,h)
   filecheck(base/'model.py',d['model_sha256'])
   cm=base/'artifacts'/d['circuit_manifest_sha256']/'manifest.json';filecheck(cm,d['circuit_manifest_sha256'])
